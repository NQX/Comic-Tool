...
ipcMain.on('start-convert', (event, arg) => {
  console.log('start qual', arg.quality)
  quality = arg.quality
...
function createFolders() {
  fs.exists(app.getPath('documents') + '/tmp', (exists) => {
    if (!exists) {
      fs.mkdirSync(app.getPath('documents') + '/tmp', () => {});
    }
  });

  fs.exists(app.getPath('documents') + '/webp', (exists) => {
    if (!exists) {
      fs.mkdirSync(app.getPath('documents') + '/webp', () => {});
    }
  });
...
function convertBatch(file) {
  let dir = app.getPath('documents') + "/tmp/"
  ...
  fs.readdir(dir, (err, files) => {
    ...
    if(hasEmptySpaces(files[0])) {
      ...
      fs.rename(dir  + files[0], dir  + removeSpaces(files[0]), function(err) {
        if(err) {
          console.log(err)
          return;
        }
...
let outputDir = app.getPath('documents') + "/webp/"
...
function unrarFile(file) {
  ...
  fs.mkdir(app.getPath('documents') + '/tmp/' + newDirectoryName, function(err) {
    if(err) console.log(err)


    
    rar.extract(app.getPath('documents') + "/tmp/" + newDirectoryName, null, function(err) {
      if(err) {
        console.log(err)
        return;
      }
...
function  packFiles() {
    zipFolder.zipFolder(app.getPath('documents')  + '/webp', app.getPath('downloads') + "/out.cbz", function(err) {
    if(err) {
      console.log('oh no!', err);
  } else {
      console.log('EXCELLENT');
      const response = dialog.showMessageBox(null, {message: 'Done!'});
      deleteFolders();
  }
  })
}
...
function deleteFolders() {
  rimraf(app.getPath('documents') + '/webp',() => {
    console.log('done')
  });

  rimraf(app.getPath('documents') + '/tmp',() => {
    console.log('done 2')
  })
}
